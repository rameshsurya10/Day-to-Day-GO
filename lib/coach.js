...
/* Renders one day row with a per-level completion count and the exact
   missed tasks, each TAGGED with its level. The tags ([L1]/[L2]/[L3])
   stop the model from confusing which tier a task belongs to, and let
   Buddy spot patterns like "workout slips on Fridays". */
function formatDay(row) {
  const done = row.done || {};
  const counts = [];
  const missed = [];
  for (const lvl of LEVELS) {
    const items = TASKS[lvl].items;
    const tag = lvl.toUpperCase(); // L1 / L2 / L3
    const doneCount = items.filter((t) => done[t.id]).length;
    counts.push(`${tag} ${doneCount}/${items.length}`);
    for (const t of items) {
      if (!done[t.id]) missed.push(`[${tag}] ${t.label}`);
    }
  }
  const tail = missed.length ? ` | missed: ${missed.join('; ')}` : ' | all tasks done';
  return `${row.date} (${weekdayOf(row.date)}) — ${row.status} ${row.score}/100 ` +
         `[${counts.join(', ')}]${tail}`;
}
